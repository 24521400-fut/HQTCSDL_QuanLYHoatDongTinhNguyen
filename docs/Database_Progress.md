...
| SP_NHAPKHO_VATPHAM | Complete | |
| SP_XUATKHO_VATPHAM | Complete | |
...
* **Trạng thái & Số lượng Chiến dịch (CHIENDICH)**:
  * `SoLuongTNVToiDa` & `SoLuongHienTai` -> Bị kiểm tra bởi `TRG_KIEMTRA_SOLUONG_DANGKY` và `SF_CHECK_DK_THAMGIA`. `SoLuongHienTai` sẽ tự động tăng/giảm thông qua `TRG_CAPNHAT_SOLUONG_TNV` (KHÔNG tự insert hardcode `SoLuongHienTai` khi làm seed data, hãy gọi `SP_TNV_DANGKY_THAMGIA`).
...
* **Hoạt động tình nguyện (THAMGIATNV & DIEMDANH)**:
  ...
  * Tổng giờ của 1 TNV được tính từ tổng `DiemDanh.SoGioGhiNhan`.

* **Vật phẩm (LOAIVATPHAM)**:
  * `SoLuongTon` -> Tự động tính toán khi gọi `SP_NHAPKHO_VATPHAM` và `SP_XUATKHO_VATPHAM` (Không hardcode update tồn kho trực tiếp).
